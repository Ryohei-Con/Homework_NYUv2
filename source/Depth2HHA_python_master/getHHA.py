...
from Depth2HHA_python_master.utils.rgbd_util import *
from Depth2HHA_python_master.utils.getCameraParam import *
...
def wrap_getHHA(image_array):
    camera_matrix = getCameraParam('color')

    # make sure that the image is in 'meter'
    # print('max gray value: ', np.max(image_array))
    hha = getHHA(camera_matrix, image_array, image_array)
    return hha
